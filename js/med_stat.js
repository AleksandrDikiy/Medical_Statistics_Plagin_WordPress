...
(function () {
    ...
    function loadOrders(page) {
        page = page || 1; State.currentPage = page;
        jq.post(medStat.ajaxUrl, {
            action: 'med_stat_get_orders', nonce: medStat.nonce, page: page,
            date_from: el.dateFrom ? el.dateFrom.value : '',
            date_to:   el.dateTo   ? el.dateTo.value   : '',
            search:    el.search   ? el.search.value   : '',
        })
        .done(function (res) {
            if (!res.success) { showToast(res.data && res.data.message || medStat.i18n.noResults, 'error'); renderOrderList([]); return; }
    ...
    function renderOrderList(orders) {
        ...
        if (!orders.length) {
            var p = document.createElement('p'); p.className = 'ms-empty'; p.textContent = medStat.i18n.noResults;
            el.orderList.appendChild(p); return;
        }
    ...
    function loadOrder(orderId) {
        ...
        .done(function (res) {
            if (!res.success) { showToast(res.data && res.data.message || medStat.i18n.importFail, 'error'); return; }
    ...
    function renderMeasurements(measurements) {
        ...
        measurements.forEach(function (m) {
            ...
            if (m.min !== null && m.max !== null) refEl.textContent = cleanNum(m.min) + ' – ' + cleanNum(m.max);
            else { refEl.textContent = '—'; var ru = clone.querySelector('.ms-col--ref .ms-unit'); if (ru) ru.remove(); }
    ...
    function deleteOrder(orderId) {
        ...
        .done(function (res) {
            if (!res.success) { showToast(res.data && res.data.message || 'Помилка', 'error'); return; }
    ...
    function saveModal() {
        ...
        jq.post(medStat.ajaxUrl, {
            action: 'med_stat_edit_row', nonce: medStat.nonce,
            meas_id:      measId, ind_id: indId,
    ...
    /* ══════════════════════════════════════════
       ADD MODAL — показники autocomplete
       ══════════════════════════════════════════ */
    function openAddModal() {
        if (!el.addModal) return;
        // Очищуємо рядки і додаємо перший
    ...
    function addMeasRow() {
        ...
        // Autocomplete для поля пошуку показника
        ...
        row.querySelector('.ms-row-del-btn').addEventListener('click', function () {
            row.remove();
        });
    ...
    function saveAddOrder() {
        ...
        jq.post(medStat.ajaxUrl, {
            action: 'med_stat_add_order', nonce: medStat.nonce,
            order_number: orderNum, collection_date: collection,
            doctor_name: doctor, branch_info: branch,
